...
export const updatePosition = (config: PositionConfig) => {
    ...
    if (position === "cursor") {
        ...
        const lineHeight = parseInt(window.getComputedStyle(input).lineHeight) || parseInt(window.getComputedStyle(input).fontSize) || 16;
        ...
        const shouldPositionAbove = spaceBelow < containerHeight + VERTICAL_SPACING && spaceAbove >= containerHeight + VERTICAL_SPACING;
